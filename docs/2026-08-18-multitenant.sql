...
select c.id, c.created_by, case when lower(coalesce(u.email, '')) = 'comercial.metrik.ai@gmail.com' then 'GLOBAL_ADMIN'::public.company_member_role else 'COMPANY_ADMIN'::public.company_member_role end, 'ACTIVE'::public.company_member_status
...
as $$ select lower(coalesce(auth.jwt()->>'email', '')) = 'comercial.metrik.ai@gmail.com' $$;
