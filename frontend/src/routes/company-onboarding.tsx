...
function CompanyOnboardingPage() {
  ...
  async function lookupCnpj() {
    ...
    setCnpjLoading(true); setCnpjConfirmed(false); setError(null);
    try {
      const { data } = await apiFetch<{ data: { cnpj: string; legalName: string; tradeName: string | null; address: string | null; city: string | null; state: string | null; postalCode: string | null; phone: string | null; status: string } }>(`/company-signup/cnpj/${digits}`);
      setField('legalName', data.legalName); if (data.tradeName) setField('tradeName', data.tradeName); if (data.address) setField('address', data.address); if (data.city) setField('city', data.city); if (data.state) setField('state', data.state); if (data.postalCode) setField('postalCode', data.postalCode); if (data.phone) setField('companyPhone', data.phone); setCnpjConfirmed(true);
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Não foi possível validar o CNPJ'); } finally { setCnpjLoading(false); }
  }
  ...
  async function submit() {
    ...
    if (!cnpjConfirmed) { setError('Valide o CNPJ antes de concluir o cadastro.'); return; }
