...
import { Building2, CircleHelp, LockKeyhole, Monitor, Moon, SlidersHorizontal, Sun, UserRound } from "lucide-react";
...
export function SettingsModalContent() {
  ...
  const [theme, setTheme] = useState<ThemePreference>(() => readPreference("metrik:theme", "system"));
  ...
  useEffect(() => {
    applyTheme(theme);
    window.localStorage.setItem("metrik:theme", theme);
...
function ThemeButton({ icon: Icon, label, selected, onClick }: { icon: typeof Sun; label: string; selected: boolean; onClick: () => void }) {
  return <button type="button" onClick={onClick} className={`flex flex-col items-center gap-1.5 rounded-xl border px-3 py-3 text-[11px] font-medium transition ${selected ? "border-violet-500 bg-violet-50 text-violet-700 ring-2 ring-violet-100" : "border-slate-200 text-slate-500 hover:border-slate-300 hover:bg-slate-50"}`}><Icon className="size-4" />{label}</button>;
}
