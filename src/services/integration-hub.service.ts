...
export async function registerAdAccount(userId: string, companyId: string, input: { platform: 'GOOGLE_ADS' | 'META_ADS'; externalAccountId: string; name: string }) {
  ...
  const supabase = getSupabaseAdmin();
  const { data: existingConnection, error: connectionError } = await supabase.from('integration_connections').select('id').eq('company_id', companyId).eq('provider', provider).maybeSingle();
  if (connectionError) throw new AppError(`Não foi possível localizar a conexão: ${connectionError.message}`, 500);
  const connection = existingConnection ?? await createConnection(userId, companyId, { provider, authType: 'OAUTH2', workspaceName: name, status: 'DRAFT' });
  const { data, error } = await supabase.from('ad_platform_accounts').upsert({ company_id: companyId, connection_id: connection.id, platform: input.platform, external_account_id: externalAccountId, name, status: 'ACTIVE', updated_at: new Date().toISOString() }, { onConflict: 'company_id,platform,external_account_id' }).select('id,company_id,connection_id,platform,external_account_id,name,currency_code,timezone,status,last_synced_at,created_at,updated_at').single();
