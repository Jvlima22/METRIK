...
import { createConnection, deleteConnection, integrationCatalog, listAdminAdAccounts, listConnectionAccounts, listConnections, listConnectionSyncRuns, registerAdAccount, testConnection, updateConnectionStatus } from '../services/integration-hub.service';
...
export async function getAdminAdAccounts(_req: Request, res: Response, next: NextFunction) { try { res.json({ data: await listAdminAdAccounts() }); } catch (error) { next(error); } }
...
export async function postAdAccount(req: Request, res: Response, next: NextFunction) { try { const body = req.body ?? {}; if (body.platform !== 'GOOGLE_ADS' && body.platform !== 'META_ADS') throw new AppError('platform deve ser GOOGLE_ADS ou META_ADS', 400); const { userId, companyId } = context(req); res.status(201).json({ data: await registerAdAccount(userId, companyId, { platform: body.platform, externalAccountId: String(body.externalAccountId ?? ''), name: String(body.name ?? '') }) }); } catch (error) { next(error); } }
